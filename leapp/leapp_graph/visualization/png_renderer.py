...
def _draw_port(
    ...
    if visible_kind:
        kind_x = port.rect.x + _NODE_PADDING if port.side == "input" else port.rect.x + port.rect.width - _NODE_PADDING
        kind_anchor = "ls" if port.side == "input" else "rs"
        _draw_text(draw, Point(kind_x, port.rect.y + 50.0), visible_kind, fill=accent, font=fonts["label"], anchor=kind_anchor)

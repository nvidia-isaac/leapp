...
def _marker(marker_id: str, color: str) -> str:
    return (
        f'<marker id="{marker_id}" markerWidth="10" markerHeight="8" refX="10" refY="4" orient="auto" markerUnits="userSpaceOnUse" overflow="visible">'
        f'<path d="M 0 0 L 10 4 L 0 8 z" fill="{color}" />'
        "</marker>"
    )
...
def _render_port_kind(port: PortGeometry, accent: str) -> str:
    ...
    if not visible_kind:
        return ""
    x = port.rect.x + 14.0 if port.side == "input" else port.rect.x + port.rect.width - 14.0
    text_anchor = "start" if port.side == "input" else "end"
    return (
        f'<text x="{_fmt(x)}" y="{_fmt(port.rect.y + 50.0)}" fill="{accent}" font-family="{escape(_FONT_FAMILY, quote=True)}" font-size="12" font-weight="600" text-anchor="{text_anchor}">{escape(visible_kind)}</text>'
    )

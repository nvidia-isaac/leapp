...
//
// CUDA-resident variant of the embedded WrpRunner custom op.
//
// This is a drop-in replacement for ../onnx_embedded_wrp/wrp_runner_op.cc that
// keeps every per-node tensor on the GPU. The original prototype registers on
// the CPU EP and uses wp_apic_set_param / wp_apic_get_param, which copy host
// <-> device on every node; in a multi-node graph that stacks ~7 PCIe transfers
// plus a CPU/CUDA EP boundary (ORT-inserted Memcpy nodes) per inference and
// blocks CUDA graph capture.
//
// This version instead:
//   - reports "CUDAExecutionProvider" so the whole graph lands on CUDA (no
//     boundary Memcpy nodes, CUDA-graph eligible),
//   - marks only the bundle initializer as a CPU input (it must be read on the
//     host to extract the .wrp), while data inputs/outputs stay device-resident,
//   - moves data with cudaMemcpyAsync device->device on ORT's compute stream and
//     launches the APIC graph on that same stream, so there are no host
//     round-trips and no full-device synchronizations on the hot path.
//
// The op domain/name match the original (com.nvidia.warp::WrpRunner), so the
// same embedded ONNX model loads against either library.
...
// Forward-declare the handful of CUDA runtime entry points we need. We avoid
// including <cuda_runtime_api.h> because it redefines float4 etc. already
// provided by warp.h's native headers. Symbols come from CUDA::cudart at link.
...
// -----------------------------------------------------------------------------
// Embedded bundle archive handling (identical WRPB format to the CPU prototype).
// -----------------------------------------------------------------------------
...
struct WrpRunnerKernel {
    ...
    // Read the bundle bytes from the (CPU) last input, extract, and load the
    // APIC graph. Done once per kernel instance on first Compute.
    ...
    void Compute(OrtKernelContext* ctx) {
        ...
        // Run all data movement + the APIC launch on ORT's compute stream so the
        // node stays ordered with the rest of the CUDA graph and needs no full
